...

{
  ...
  printf("\n This run will have\n Producers = %d\n Consumers=%d\n QueueDepth = %d\n Number Of Dozens = %d\n Flavor=%d\n Thread Scope =%s\n Number of CPUs=%d\n",NUMPRODUCERS,NUMCONSUMERS,NUMSLOTS,NUM_DOZ_TO_CONS,NUMFLAVORS,"System",get_nprocs());
  ...
  if((cn = open("./system_times",O_WRONLY|O_CREAT|O_APPEND, 0644)) == -1)
  {
    perror("can not open sys time file ");
    exit(1);
  }
  sprintf(msg, "\n\nSystem_Scope: Elapsed consumer time is %f sec\n", etime);
