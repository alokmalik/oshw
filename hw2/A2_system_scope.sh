...
# usage: a2_system_scope.sh  number_of_runs_integer
# so from the shell prompt:  $ ./a2_system_scope.sh 10
# will run the multi threaded process named   pctsys
# 10 times and report how many of the runs ended in deadlock

if [ -z $1 ]
then
echo "USAGE: a2_system_scope.sh  number_of_runs_integer"
...
echo " "
./pctsys
pid=`ps | grep pctsys | awk '{print $1}'`
# echo JOB AND PID IS $pid
sleep 10
pid=`ps | grep 'pctsys' | awk '{print $1}'`
# if  ps | grep 'pctsys' > /dev/null
if [ -n "$pid" ]
then
...
echo "        $1 LOOPS and $count DEADLOCK(S)" >> sys_DL
echo
